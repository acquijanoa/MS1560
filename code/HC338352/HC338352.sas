...
proc mi data = &data nimpute = 0 displaypattern=nomeans;
	...
	var birthwt_ga_z parity_v1 povpct accult_mesa marital_status employedyn education_c3 yrsus_c3 income_c2 n_hc;
run;
...
* Physical and mental health, anthropometry;
 title1 J=center height=12pt bold font='TIMES ROMAN' "MS#1560 SOL/FLOR preconceptional factors and child weight" ;
		 ...
		 footnote2 J=left height=10pt font='TIMES ROMAN' "^S={leftmargin=&lf_margin rightmargin=&rg_margin} Abbrev: weight-for-age z-score (WAZ), 10-items state-trait anxiety inventory (STAI10), waist girth (ANTA10a), Center for Epidemiologic Studies Depression Scale (CESD-10)";
		 footnote3 J=center height=10pt font='TIMES ROMAN' "&sysdate, &systime -- &job (&prg) using V1 and FLOR inv_use data";
proc mi data = &data nimpute = 0 displaypattern=nomeans;
	ods select MissPattern;
...
* Maternal Health behaviors;
 title1 J=center height=12pt bold font='TIMES ROMAN' "MS#1560 SOL/FLOR preconceptional factors and child weight" ;
         ...
         footnote1 J=left height=10pt font='TIMES ROMAN' "^S={leftmargin=&lf_margin rightmargin=&rg_margin} Abbrev: weight-for-age z-score (WAZ), health eating index 2010 (HEI2010), sleep duration (SLPDUR).";
