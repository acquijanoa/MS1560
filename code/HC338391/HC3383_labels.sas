...
%MACRO labels();
	...
	* Model 1 variables;
	if variable = 'AGE' then do; label = "{\b Maternal age \b0 \li250}"; order = 1; end;
	...
	if variable = 'BKGRD1_C7NOMISS_MORE THAN ONE' THEN DO; LABEL = "^S={indent=2mm} More than one heritage"; ORDER = 2.5; END;
	if variable = 'BKGRD1_C7NOMISS_BK_OTHER' THEN DO; LABEL = "^S={indent=2mm} other heritage"; ORDER = 2.6; END;
	if variable = "N_HC_YES" THEN DO; LABEL = "{\b Health Insurance \line \b0 \li250   Yes}"; ORDER=3; ESTIMATE = 99; STD=99; pv = 5; END;
	if variable = "N_HC_NO" THEN DO; LABEL = "^S={indent=2mm} No"; ORDER=3.1; END; 
	...
	if variable = 'PARITY_V1' then do; label = "{\b Number of live births \b0 \li250}"; order = 9; end;
