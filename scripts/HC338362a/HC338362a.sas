 ...
 **********************************************************
 *                                                        *
 *  PROGRAM NAME: HC338362a.sas
 *
 *  PROGRAMMER: Alvaro Quijano (AQ)
 *
 *  DESCRIPTION: Same pooled GENMOD structure as HC338354a (job 54a);
 *               outcome WAZ; all models add child height-for-age
 *               z-score (HAZ) as a covariate.
 *
 * ---------------------------------------------------------
 *
 *  JOB NUMBER: HC338362a
 *
 *  PREVIOUS JOB: HC338362
 *
 *  LANGUAGE: SAS 9.4
 *
 *  VERSION CONTROL:
 *					13apr26: Create from HC338354a (job 54a); add HAZ covariate;
 *							 init macros &req, &job (&req.62a), &datefile;
...
proc genmod data=&impdb;
	...
	model waz=centernum yrs_btwn_v1flor age haz bkgrd1_c7nomiss n_hc education_c3
...
proc report data=db_join;
	title j=center height=&fs font='times roman' bold
		"^S={leftmargin=&lft_mgn rightmargin=&rgt_mgn}Table 2a. Maternal preconception socio-behavioral factors and child's weight-for-age z-score, HCHS/SOL FLOR Ancillary Study (n=%qtrim(&n_ids))";
	footnote1 J=left HEIGHT=&fs_titles FONT='times roman'
		"^S={leftmargin=&lft_mgn rightmargin=&rgt_mgn}Abbreviations: CI, confidence interval; FLOR, Family Lifestyle Outcomes Research; PA, physical activity; HAZ, height-for-age z-score.";
	footnote2 J=left HEIGHT=&fs_titles FONT='times roman'
		"^S={leftmargin=&lft_mgn rightmargin=&rgt_mgn}Model 1: Sociodemographic & acculturation predictors adjusted by field center, years between baseline and FLOR visit. All models adjust for child height-for-age z-score (HAZ).";
