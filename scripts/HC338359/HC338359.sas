...
proc printto log="&homepath.\scripts\HC338359\HC338359_&sysdate..log" 
	print = "&homepath.\scripts\HC338359\HC338359_&sysdate..lst" new; 
run;
...
*               BMI-for-age percentile (BMIPCT) across
*               imputations; Table 2 model estimates (1-4).
...
*
*  VERSION CONTROL: 
*					30mar26: Create from HC338354 (job 54);
*							 response is BMIPCT instead of WAZ.
*					30mar26: Table footnote abbreviations aligned with job 60.
*
* ----------------------------------------------------------
*
*  INPUT: 
...
proc genmod data = &impdb;
	...
	model bmipct = centernum yrs_btwn_v1flor age bkgrd1_c7nomiss n_hc education_c3 parity_v1 employedyn marital_status yrsus_c3 / d=normal;
...
proc genmod data = &impdb;
	...
	model bmipct = centernum yrs_btwn_v1flor age bkgrd1_c7nomiss n_hc education_c3 parity_v1 employedyn marital_status yrsus_c3
...
%get_all_partial_r2(impdata=&impdb, class_vars=&pr2_class_vars,
	cont_vars=&pr2_cont_vars, outds=partial_r2_summary, outcome=bmipct);
...
proc report data = db_join;
	title j=center height=&fs font='times roman' bold "^S={leftmargin=&lft_mgn rightmargin=&rgt_mgn}Table 2. Maternal preconception socio-behavioral factors and child's BMI-for-age percentile, HCHS/SOL FLOR Ancillary Study (n=%qtrim(&n_ids))";
	footnote1 J=LEFT HEIGHT=&fs_titles FONT='times roman' "^S={leftmargin=&lft_mgn rightmargin=&rgt_mgn}Abbreviations: FLOR, Family Lifestyle Outcomes Research.";
