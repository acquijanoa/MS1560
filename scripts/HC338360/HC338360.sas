...
*               BMI-for-age percentile (BMIPCT, 0-100), dist=bin,
*               link=logit, GEE with robust (empirical) variance.
*               Same covariate sets as job 59 (HC338359).
*               SAS may NOTE non-integer events/trials.
*
* ---------------------------------------------------------
*
*  JOB NUMBER: HC338360
...
*
*  VERSION CONTROL:
*					31mar26: Create from HC338359 (job 59);
...
* Fractional logit: model BMIPCT/100 as binomial proportion with trials=100;
data impwork;
	set &impdb;
	trial_wt = 100;
...
%macro process_imputed2(in_db=, out_db=, model=);
	...
	* Reshape the dataset to include effect names;
	data &in_db.;
	    set &in_db.;
	    length ParmName $32 EffectName $50;
	    /* Harmonize parameter naming with GEEEmpPEst (Parm or Parameter) */
	    ParmName = coalescec(Parm, Parameter);
	    if missing(Level1) then EffectName = ParmName;
	    else EffectName = catx('_', ParmName, Level1);
	run;
...
proc report data = db_join;
	title j=center height=&fs font='times roman' bold "^S={leftmargin=&lft_mgn rightmargin=&rgt_mgn}Table 2. Maternal preconception socio-behavioral factors and child's BMI-for-age percentile (fractional probit), HCHS/SOL FLOR Ancillary Study (n=%qtrim(&n_ids))";
	...
	footnote7 J=LEFT HEIGHT=10pt FONT='times roman' "{\line \line Job &job run by &PRG using FLOR data on %sysfunc(today(), date9.) at %qtrim(%sysfunc(time(), timeampm.))}";
