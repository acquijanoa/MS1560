 ...
 **********************************************************
 *                                                        *
 *  PROGRAM NAME: HC338363a.sas
 *
 *  PROGRAMMER: Alvaro Quijano (AQ)
 *
 *  DESCRIPTION: Pooled linear GENMOD (normal) for child
 *               birth weight-for-gestational-age z-score
 *               (BIRTHWT_GA_Z) in the PRS-complete sample;
 *               Table 2a model estimates (Models 1-4).
 *
 * ---------------------------------------------------------
 *
 *  JOB NUMBER: HC338363a
 *
 *  PREVIOUS JOB: HC338363
 *
 *  LANGUAGE: SAS 9.4
 *
 *  VERSION CONTROL:
 *					27apr26: Create from HC338354a (job 54a);
...
proc genmod data=&impdb;
	...
	model birthwt_ga_z=centernum yrs_btwn_v1flor age bkgrd1_c7nomiss n_hc education_c3
...
%get_all_partial_r2(impdata=&impdb, class_vars=&pr2_class_vars,
	cont_vars=&pr2_cont_vars, outds=partial_r2_summary, outcome=birthwt_ga_z);
...
proc report data=db_join;
	title j=center height=&fs font='times roman' bold
		"^S={leftmargin=&lft_mgn rightmargin=&rgt_mgn}Table 2a. Maternal preconception socio-behavioral factors and child's birth weight-for-gestational-age z-score, HCHS/SOL FLOR Ancillary Study (n=%qtrim(&n_ids))";
