...
**********************************************************
*                                                        *
*  PROGRAM NAME: HC338362.sas
*                                       
*  PROGRAMMER: Alvaro Quijano (AQ)
*
*  DESCRIPTION: Same pooled GENMOD structure as HC338354 (job 54);
*               outcome WAZ; all models add child height-for-age
*               z-score (HAZ) as a covariate.
...
*
*  VERSION CONTROL: 
*					13apr26: Create from HC338354 (job 54); add HAZ covariate;
*							 init macros &req, &job (&req.62), &datefile;
...
%let pr2_class_vars = bkgrd1_c7nomiss marital_status employedyn education_c3 n_hc
			...
			centernum;
%let pr2_cont_vars = age haz parity_v1 slpdur child_prs_bmi_a yrs_btwn_v1flor;
...
proc genmod data = &impdb;
	...
	model waz = centernum yrs_btwn_v1flor age haz bkgrd1_c7nomiss n_hc education_c3 parity_v1 employedyn marital_status yrsus_c3 / d=normal;
...
proc genmod data = &impdb;
	...
	model waz = centernum yrs_btwn_v1flor age haz bkgrd1_c7nomiss n_hc education_c3 parity_v1 employedyn marital_status yrsus_c3
...
%get_all_partial_r2(impdata=&impdb, class_vars=&pr2_class_vars,
	cont_vars=&pr2_cont_vars, outds=partial_r2_summary, outcome=waz);
...
proc report data = db_join;
	...
	footnote1 J=LEFT HEIGHT=&fs_titles FONT='times roman' "^S={leftmargin=&lft_mgn rightmargin=&rgt_mgn}Abbreviations: CI, confidence interval; FLOR, Family Lifestyle Outcomes Research; PA, physical activity; HAZ, height-for-age z-score.";
	footnote2 J=LEFT HEIGHT=&fs_titles FONT='times roman' "^S={leftmargin=&lft_mgn rightmargin=&rgt_mgn}Model 1: Sociodemographic & acculturation predictors adjusted by field center, years between baseline and FLOR visit. All models adjust for child height-for-age z-score (HAZ).";
