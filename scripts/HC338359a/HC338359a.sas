...
proc printto log="&homepath.\scripts\HC338359a\HC338359a_&sysdate..log" print=
	"&homepath.\scripts\HC338359a\HC338359a_&sysdate..lst" new;
run;
 ...
 *               BMI-for-age percentile (BMIPCT) in the
 *               PRS-complete imputed sample; Table 2a.
 *
 * ---------------------------------------------------------
 *
 *  JOB NUMBER: HC338359a
 *
 *  PREVIOUS JOB: HC338359
 *
 *  LANGUAGE: SAS 9.4
 *
 *  VERSION CONTROL:
 *					30mar26: Create from HC338354a (job 54a);
 *							 response is BMIPCT instead of WAZ.
 *					30mar26: Match HC338359 (job 59): landscape orientation.
...
proc genmod data=&impdb;
	...
	model bmipct=centernum yrs_btwn_v1flor age bkgrd1_c7nomiss n_hc education_c3
...
proc report data=db_join;
	title j=center height=&fs font='times roman' bold
		"^S={leftmargin=&lft_mgn rightmargin=&rgt_mgn}Table 2a. Maternal preconception socio-behavioral factors and child's BMI-for-age percentile, HCHS/SOL FLOR Ancillary Study (n=%qtrim(&n_ids))";
	footnote1 J=left HEIGHT=&fs_titles FONT='times roman'
		"^S={leftmargin=&lft_mgn rightmargin=&rgt_mgn}Abbreviations: FLOR, Family Lifestyle Outcomes Research.";
