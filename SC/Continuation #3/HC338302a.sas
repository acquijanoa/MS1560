******************************************************************
  ...
  DATE:          4/23/2025
-----------------------------------------------------------------
  JOB NUMBER:    HC338302a

  DESCRIPTION:   Create Table 1a. Maternal Preconception and Child Characteristics among HCHS/SOL women with a 
...
proc freq data=derive2(where =(&denomlogic)) noprint;
    tables flor_dyad*&var / missing outpct
        out=myout(keep=flor_dyad &var count pct_row where=(&var=&varlevel));
run;
...
proc transpose data=cp prefix=flor_dyad suffix=_p out=&var.pct (drop=_name_);
    by &var;
    id flor_dyad;
    var cp_pct;
run;
...
length vartext $200 var $50;
    ...
    rename
    flor_dyad0_n = nonflor_n
    flor_dyad0_p = nonflor_p
    flor_dyad1_n = flor_n
    flor_dyad1_p = flor_p
    ;
run;
...
* Write RTF report;
ods rtf file="&workspace\&job._T1a_&sysdate..rtf" bodytitle style=journal;
ods escapechar="^"; 
title1 "Table 1a. Maternal Preconception and Child Characteristics among HCHS/SOL women with a child born ^n between baseline (2008-11) and Visit 2 (2014-17), by FLOR participation.";
...
columns vartext ("FLOR dyads ^n (N=&flor_ct)" flor_n flor_p)  ("Non-FLOR dyads ^n (N=&nonflor_ct)" nonflor_n nonflor_p) pvalue;
